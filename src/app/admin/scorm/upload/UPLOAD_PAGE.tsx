  ...
  return (
    <div className="upload-root">
      ...
      <main className="upload-main">
          ...
          <div className="progress-card">
            ...
            <div className="progress-status">
              <span className="status-label">
                {stage === "uploading" ? `Uploading & extracting… ${Math.round(progress)}%` : "Validating SCORM manifest…"}
              </span>
        ...
        {stage === "publishing" && (
          <div className="progress-card">
            <div className="result-header">
              ...
              <div>
                <h2 className="result-title">Publishing to catalog…</h2>
                <p className="result-sub">Adding &ldquo;{courseTitle}&rdquo; to the course catalog</p>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="result-card">
            <div className="result-header">
              ...
              <div>
                ...
                <p className="result-sub">Ready to publish to the course catalog</p>
              </div>
            </div>
        ...
        <aside className="requirements-panel">
          ...
          <ul className="req-list plain">
            <li>Package extracted to <code>public/scorm/</code></li>
          ...
          <ul className="req-list plain">
            ...
            <li>
              Run <code>npx prisma db push</code> to init DB
            </li>
            <li>
              Set <code>DATABASE_URL=file:./dev.db</code>
            </li>
          </ul>
        </aside>
      </main>
      ...
      <style jsx>{`
        ...
        :global(body) { background: #0f1117; color: #e2e8f0; font-family: 'IBM Plex Sans', sans-serif; min-height: 100vh; }
        .upload-root { min-height: 100vh; background: #0f1117; }
        .page-header { border-bottom: 1px solid #1e2433; padding: 28px 40px 24px; background: #0c0e14; }
        .breadcrumb { font-size: 13px; color: #4a5568; margin-bottom: 12px; display: flex; gap: 8px; align-items: center; }
        .breadcrumb a { color: #5a7aff; text-decoration: none; }
        .breadcrumb a:hover { text-decoration: underline; }
        .sep { color: #2d3748; }
        .page-title { font-size: 26px; font-weight: 600; letter-spacing: -0.4px; color: #f0f4ff; margin-bottom: 6px; }
        .page-sub { font-size: 14px; color: #4a5568; }
        .upload-main { display: grid; grid-template-columns: 1fr 280px; gap: 24px; padding: 32px 40px; max-width: 1100px; }
        .drop-zone { border: 2px dashed #2a3347; border-radius: 8px; background: #111520; padding: 72px 40px; display: flex; flex-direction: column; align-items: center; gap: 12px; cursor: pointer; transition: all 0.2s ease; position: relative; overflow: hidden; }
        .drop-zone::before { content: ''; position: absolute; inset: 0; background: radial-gradient(circle at 50% 60%, rgba(90,122,255,0.04) 0%, transparent 70%); pointer-events: none; }
        ...
        .badge { font-size: 11px; font-family: 'IBM Plex Mono', monospace; padding: 3px 10px; border-radius: 3px; background: #1a2035; border: 1px solid #2a3347; color: #6b7fa8; letter-spacing: 0.3px; }
        .progress-card { background: #111520; border: 1px solid #1e2433; border-radius: 8px; padding: 32px; display: flex; flex-direction: column; gap: 24px; }
        ...
        .progress-track { height: 4px; background: #1e2433; border-radius: 2px; overflow: hidden; }
        .progress-fill { height: 100%; background: linear-gradient(90deg, #4060dd, #5a7aff); border-radius: 2px; transition: width 0.3s ease; }
        .progress-status { display: flex; align-items: center; justify-content: space-between; font-size: 13px; color: #4a5568; }
        .status-label { color: #7a90bc; }
        .spinner { width: 16px; height: 16px; border: 2px solid #2a3347; border-top-color: #5a7aff; border-radius: 50%; animation: spin 0.6s linear infinite; flex-shrink: 0; }
        @keyframes spin { to { transform: rotate(360deg); } }
        .progress-steps { display: flex; align-items: center; gap: 0; }
        .step-line { flex: 1; height: 1px; background: #1e2433; }
        .result-card { background: #111520; border: 1px solid #1e2433; border-radius: 8px; padding: 32px; display: flex; flex-direction: column; gap: 28px; }
        .result-header { display: flex; align-items: flex-start; gap: 16px; }
        .result-icon { width: 44px; height: 44px; border-radius: 6px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
        ...
        .validation-grid { display: flex; flex-direction: column; border: 1px solid #1e2433; border-radius: 6px; overflow: hidden; }
        ...
        .form-input { background: #0c0e14; border: 1px solid #2a3347; border-radius: 5px; padding: 10px 14px; font-size: 15px; color: #e2e8f0; font-family: 'IBM Plex Sans', sans-serif; outline: none; transition: border-color 0.15s; }
        .form-input:focus { border-color: #5a7aff; }
        .form-hint { font-size: 12px; color: #3a4a68; }
        .result-actions { display: flex; gap: 12px; }
        .btn-primary { background: #5a7aff; color: #fff; border: none; padding: 10px 22px; border-radius: 5px; font-size: 14px; font-weight: 500; font-family: 'IBM Plex Sans', sans-serif; cursor: pointer; transition: background 0.15s, opacity 0.15s; }
        .btn-primary:hover:not(:disabled) { background: #4060dd; }
        .btn-primary:disabled { opacity: 0.4; cursor: not-allowed; }
        .btn-ghost { background: transparent; color: #7a90bc; border: 1px solid #2a3347; padding: 10px 22px; border-radius: 5px; font-size: 14px; font-family: 'IBM Plex Sans', sans-serif; cursor: pointer; transition: all 0.15s; }
        .btn-ghost:hover { border-color: #5a7aff; color: #c5d0e8; }
        .requirements-panel { background: #0c0e14; border: 1px solid #1a2035; border-radius: 8px; padding: 24px; height: fit-content; position: sticky; top: 24px; }
        .req-title { font-size: 11px; font-weight: 600; letter-spacing: 1px; text-transform: uppercase; color: #3a4a68; margin-bottom: 14px; }
        .req-list { list-style: none; display: flex; flex-direction: column; gap: 10px; }
        .req-list.plain li { font-size: 12px; color: #4a5568; padding-left: 12px; border-left: 2px solid #1e2433; line-height: 1.6; font-family: 'IBM Plex Mono', monospace; }
        .req-list.plain code { background: #1a2035; padding: 1px 4px; border-radius: 3px; font-size: 11px; }
...
function Step({ done, active, label }: { done: boolean; active: boolean; label: string }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
      <div style={{ width: 28, height: 28, borderRadius: "50%", border: `2px solid ${done ? "#4ade80" : active ? "#5a7aff" : "#2a3347"}`, background: done ? "rgba(34,197,94,0.12)" : active ? "rgba(90,122,255,0.12)" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.2s" }}>
        {done ? (
      ...
      <span style={{ fontSize: 11, color: done ? "#4ade80" : active ? "#7a90bc" : "#2a3347", fontFamily: "'IBM Plex Mono', monospace" }}>{label}</span>
    </div>
  );
}
...
function ValidationRow({ label, value, tag, mono }: { label: string; value: string; tag?: boolean; mono?: boolean }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 16px", borderBottom: "1px solid #1e2433" }}>
      <span style={{ fontSize: 13, color: "#4a5568" }}>{label}</span>
      {tag ? (
        <span style={{ fontSize: 11, fontFamily: "'IBM Plex Mono', monospace", padding: "3px 10px", borderRadius: 3, background: "rgba(90,122,255,0.12)", border: "1px solid rgba(90,122,255,0.25)", color: "#8099ff", letterSpacing: "0.3px" }}>{value}</span>
      ) : (
        <span style={{ fontSize: 13, color: "#9aabcc", fontFamily: mono ? "'IBM Plex Mono', monospace" : "inherit" }}>{value}</span>
      )}
    </div>
  );
}
